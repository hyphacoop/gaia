...
const (
	...
	distributionSubmissionDeposit  = "100"
...
func (s *DistributionSuite) SetupSuite() {
	...
	node.StakingDelegate(s.GetContext(), s.DelegatorWallet.KeyName(), s.Chain.ValidatorWallets[0].ValoperAddress, string(distributionStakeAmount)+s.Chain.Config().Denom)
	node.StakingDelegate(s.GetContext(), s.DelegatorWallet2.KeyName(), s.Chain.ValidatorWallets[0].ValoperAddress, string(distributionStakeAmount)+s.Chain.Config().Denom)
}
